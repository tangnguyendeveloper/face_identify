...
from .model_path import pnet_path, onet_path, rnet_path
import cv2
import numpy as np
...
class MTCNN(object):
    ...
    def __init__(self, min_face_size: int = 20, steps_threshold: list = None,
        ...
        self._pnet = LoadModelTFlite(pnet_path)
        self._rnet = LoadModelTFlite(rnet_path)
        self._onet = LoadModelTFlite(onet_path)
    ...
    def __stage1(self, image, scales: list, stage_status: StageStatus):
        ...
        for scale in scales:
            ...
            out = TFlitePredict(self._pnet, img_y.astype(np.float32))
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        out = TFlitePredict(self._rnet, tempimg1.astype(np.float32))
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        out = TFlitePredict(self._onet, tempimg1.astype(np.float32))
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
